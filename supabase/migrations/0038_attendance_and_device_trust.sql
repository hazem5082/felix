...
-- platform.create_tenant_schema() requires exactly twenty of those per
-- schema, and this function needs none of the privilege — it reads
-- branches, which branches_select already shows to every member of the
-- showroom.
--
...
--   * It adds no SECURITY DEFINER function, so assertion (f)'s twenty
--     is untouched.
...
begin
  ...
  -- 0037 is a HARD gate, not an ordering preference. Every attendance
  -- policy below calls can_act_on_branch() or can_read_branch(), and
  -- before 0037 those two raise "permission denied for schema auth" for
  -- every tenant session — so attendance would ship into a showroom
  -- where nobody can punch and the failure would look like a bug in
  -- this file rather than the one it actually is.
  if position('request.jwt.claim.sub' in platform.tenant_ddl_template()) = 0 then
    raise exception
      '0038 PRECONDITION FAILED: the branch predicates still resolve auth.* as the tenant role. Apply 0037 first, or every punch will 403.';
  end if;
end
$$;
...
--
-- NOT SECURITY DEFINER: assertion (f)'s count of twenty per schema is
-- untouched, and this needs no privilege — branches_select already
